...
int load_dat(O3Data *od, int file_id, int options)
{
  ...
  int field_object_num_id[3] = { 0, 0, 0 };
  int endianness_switch;
  int i;
  int j;
  int result;
  unsigned short short_valid = 0;
  float o3_version;
  float value;
  fzPtr *dat_in;
  int dummy;
  ...
  */
  actual_len = fzread(od->grid.start_coord, sizeof(float), 3, dat_in);
  if (actual_len != 3) {
    return PREMATURE_DAT_EOF;
  }
  fix_endianness(od->grid.start_coord, sizeof(float), 3, endianness_switch);
  actual_len = fzread(od->grid.end_coord, sizeof(float), 3, dat_in);
  if (actual_len != 3) {
    return PREMATURE_DAT_EOF;
  }
  fix_endianness(od->grid.end_coord, sizeof(float), 3, endianness_switch);
  
  /* then, read 3 int:
  - number of nodes in x, y, z directions;
  */
  actual_len = fzread(od->grid.nodes, sizeof(float), 3, dat_in);
  if (actual_len != 3) {
    return PREMATURE_DAT_EOF;
  }
  fix_endianness(od->grid.nodes, sizeof(float), 3, endianness_switch);

  /* then, read 3 float:
  - step size in x, y, z directions;
  */
  actual_len = fzread(od->grid.step, sizeof(float), 3, dat_in);
  if (actual_len != 3) {
    return PREMATURE_DAT_EOF;
  }
  fix_endianness(od->grid.step, sizeof(float), 3, endianness_switch);
  ...
  */
  ...
  if (!IS_O3A(od)) {
    ...
    if (fields_objects_x_y_vars[FIELD_NUM]
      && alloc_x_var_array(od, fields_objects_x_y_vars[FIELD_NUM])) {
      return OUT_OF_MEMORY;
    }
  }
  
  /*
  then, read an unsigned short:
  - valid analyses
  */
  if (o3_version < 2.105) {
    actual_len = fzread(&short_valid, sizeof(unsigned short), 1, dat_in);
    fix_endianness(&short_valid, sizeof(unsigned short), 1, endianness_switch);
    od->valid = (unsigned long)short_valid;
  }
  else {
    actual_len = fzread(&(od->valid), sizeof(unsigned long), 1, dat_in);
    fix_endianness(&(od->valid), sizeof(unsigned long), 1, endianness_switch);
  }
  if (actual_len != 1) {
    return PREMATURE_DAT_EOF;
  }
  ...
  */
  while (!((field_object_num_id[FIELD_NUM] ==
    ...
    */
    ...
    od->al.mol_info[field_object_num_id[OBJECT_NUM]]->object_id = field_object_num_id[OBJECT_ID];
    od->al.mol_info[field_object_num_id[OBJECT_NUM]]->struct_num = field_object_num_id[OBJECT_NUM];
    /*
  ...
  */
  if (fields_objects_x_y_vars[FIELD_NUM]) {
    if (IS_O3A(od)) {
      if (fzseek(dat_in, sizeof(unsigned short)
        * fields_objects_x_y_vars[FIELD_NUM], SEEK_CUR)) {
        return PREMATURE_DAT_EOF;
      }
    }
    else {
      actual_len = fzread(od->mel.field_attr, sizeof(unsigned short),
        fields_objects_x_y_vars[FIELD_NUM], dat_in);
      if (actual_len != fields_objects_x_y_vars[FIELD_NUM]) {
        return PREMATURE_DAT_EOF;
      }
      fix_endianness(od->mel.field_attr, sizeof(unsigned short),
        fields_objects_x_y_vars[FIELD_NUM], endianness_switch);
    }
  }
  actual_len = fzread(od->mel.object_attr, sizeof(unsigned short),
    od->grid.object_num, dat_in);
  if (actual_len != od->grid.object_num) {
    return PREMATURE_DAT_EOF;
  }
  fix_endianness(od->mel.object_attr, sizeof(unsigned short),
    od->grid.object_num, endianness_switch);
  if (o3_version > 2.03) {
    actual_len = fzread(od->mel.object_weight, sizeof(double),
      od->grid.object_num, dat_in);
    if (actual_len != od->grid.object_num) {
      return PREMATURE_DAT_EOF;
    }
    fix_endianness(od->mel.object_weight, sizeof(double),
      od->grid.object_num, endianness_switch);
  }
  for (i = 0; i < fields_objects_x_y_vars[FIELD_NUM]; ++i) {
    if (IS_O3A(od)) {
      ...
      actual_len = fzread(od->mel.x_var_attr[i], sizeof(unsigned short),
        fields_objects_x_y_vars[X_VARS_NUM], dat_in);
      if (actual_len != fields_objects_x_y_vars[X_VARS_NUM]) {
        return PREMATURE_DAT_EOF;
      }
      fix_endianness(od->mel.x_var_attr[i], sizeof(unsigned short),
  ...
  */
  ...
  if (od->y_vars) {
    ...
    */
    ...
    /*
    then, read a number of doubles equal to y_vars:
    - y_weight_coefficients;
    */
    fzread(od->mel.y_data,
    ...
    while (object_num != (od->grid.object_num - 1)) {
      ...
      */
      ...
      if ((object_num < 0) || (object_num >= od->grid.object_num)) {
        return PREMATURE_DAT_EOF;
      }
      /*
      read y_vars
    ...
    */
    actual_len = fzread(od->mel.y_var_attr, sizeof(unsigned short),
      od->y_vars, dat_in);
